...
export const metadata: Metadata = {
  ...
  description: "Recetas, guías y tips de la Escuela de Artes Dulces Casa Nelly.",
};
  ...
  return (
    <main className="bg-[#FAF7F2] min-h-screen selection:bg-[#70703A]/20">
      ...
      <Section className="pb-16 border-t border-[#5E5145]/5 pt-16">
        <h2 className="text-2xl sm:text-3xl font-bold text-[#70703A]">Guías</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
          {guias.map((g) => (
            <Link key={g.slug} href={`/recetas/${g.slug}`}>
              <Card className="rounded-3xl p-6 h-full hover:shadow-md transition-shadow">
                <div className="text-4xl" aria-hidden="true">📖</div>
                <h3 className="mt-4 text-xl font-bold text-[#70703A]">{g.titulo}</h3>
                <p className="mt-2 text-sm text-[#5E5145] leading-relaxed">{g.resumen}</p>
              </Card>
            </Link>
          ))}
        </div>
